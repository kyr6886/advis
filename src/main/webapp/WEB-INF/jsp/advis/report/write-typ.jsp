...
<body>
	<div class="container-fluid " id="scope-report" v-cloak>
...
<!-- 							</div> -->
							...
							<div class="form-check form-check-inline">
								<input class="form-check-input" type="radio"
									id="rain_150" value="150"  v-model="summary.checkedRain"> <label
									class="form-check-label" for="rain_150">누적강수 150mm 이상</label>
							</div>
							...
							<div class="form-check form-check-inline">
								<input class="form-check-input" type="radio"
									id="rain_50" value="50" v-model="summary.checkedRain"> <label
									class="form-check-label" for="rain_50">누적강수 50mm 이상</label>
							</div>
				...
				<div class="right-pan">
					...
					<div class="card card-sm" >
						...
						<h6><i class="fas fa-caret-square-right blue"></i> 현재 태풍 정보</h6>
							...
							<div class="form-group">
								<h6><i class="fas fa-caret-square-right blue"></i>유사태풍 피해사례</h6>
								<table  class="table table-bordered table-sm">
									<tr>
										<td scope="row" style="width:200px;">유사 태풍명</td>
										<td v-if="viewModel.maxDamageTyphoon_typName==''">-</td>
										<td v-else>{{viewModel.maxDamageTyphoon_typName}}</td>
									</tr>
									<tr>
										<td scope="row" style="width:200px;">인명 피해(명)</td>
										<td v-if="viewModel.maxDamageTyphoon_personDme==''">-</td>
										<td v-else>{{viewModel.maxDamageTyphoon_personDme}}</td>
									</tr>
									<tr>
										<td scope="row" >공공시설 피해(억원)</td>
										<td v-if="viewModel.maxDamageTyphoon_moneyDme.totalPub==''">-</td>
										<td v-else>{{(parseFloat(viewModel.maxDamageTyphoon_moneyDme.totalPub/100000)).toFixed(2)}}</td>
									</tr>
									<tr>
										<td scope="row">사유시설 피해(억원)</td>
										<td v-if="viewModel.maxDamageTyphoon_moneyDme.totalPri==''">-</td>
										<td v-else>{{(parseFloat(viewModel.maxDamageTyphoon_moneyDme.totalPri/100000)).toFixed(2)}}</td>
									</tr>
									<tr>
										<td scope="row">주요 피해시설</td>
										<td v-if="viewModel.maxDamageTyphoon_dmeFac==''">-</td>
										<td v-else>{{viewModel.maxDamageTyphoon_dmeFac}}</td>
									</tr>
									<tr>
										<td scope="row">주요 피해지역</td>
										<td v-if="viewModel.maxDamageTyphoon_sidoDme==''">-</td>
										<td v-else>{{viewModel.maxDamageTyphoon_sidoDme}}</td>
									</tr>
								</table>
							</div>
							
							<div class="form-group">
								<h6><i class="fas fa-caret-square-right blue"></i> 유사태풍 대응사례</h6>
								<div class="table-basic" style="max-height:425px; overflow-y: auto;">
									<table>
										<thead>
							...
							<div class="form-group">
								<h6><i class="fas fa-caret-square-right blue"></i> 유사태풍 관련 뉴스</h6>
...
<%-- 		<input type="hidden" name="paramEvtId" value="${paramEvtId}"/> --%>
	...
	<script type="text/javascript">
		function pageLoad(obj) {
			...
			obj.fnListDisEvents('NTY');
			obj.fnListCategories('NTY',"${vm.paramEvtId}");
	...
	<script	src="${pageContext.request.contextPath}/contents/advis/js/model/vue/report-controller.min.js"></script>
</body>
</html>