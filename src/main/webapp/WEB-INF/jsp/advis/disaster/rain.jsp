...
<div class="contents-title">
		...
		<ul class="tab">
			<li class="on" ><a href="/advis/disaster/rain">호우</a></li>
...
<!-- 		</div> -->
		...
		<div class="search">
				 <select  v-model="viewModel.selectTopDmeCode" class="select-box-width-120" disabled="disabled">
					<option v-for="item in viewModel.listTopDmeCode" v-bind:value='item.code' >{{item.code_name_dme}}</option>
				</select> 
				 <select v-model="viewModel.stDate" class="select-box-width-120" v-on:change='fnGetHistoryDatebyYear()'>
						<option v-for="item in viewModel.listYearsSelect" v-bind:value="item">{{item}}년  </option>
				</select>
				<select v-model="viewModel.selectDmeDate"  class="select-box-width-120"  v-on:change='fnChangeSidoList()'>
	...
	<div class="table-basic" >
		...
		<table>
				...
				<tbody >
					<tr v-if="viewModel.viewType=='person'" v-for="item in viewModel.listWholeDme">
						...
						<!-- <td><a class="btn btn-default btn-small" href="#" v-on:click="fnOnClickDetailDamageOpenWindow(item)">보기</a></td> -->
					</tr>
					<tr  v-if="viewModel.viewType!='person'" v-for="item in viewModel.listWholeDme">
						
						<td><span><a href="#" v-on:click="fnOnClickGunguDme(item, false)"><u>{{item.beg_date.substring(0,4)}} - {{item.beg_date.substring(4,6)}} - {{item.beg_date.substring(6,8)}}</u></a></span></td>
						<td>{{item.end_date.substring(0,4)}} - {{item.end_date.substring(4,6)}} - {{item.end_date.substring(6,8)}}</td>
						<td><span v-if="item.typ_name == null">{{item.damage_name}}</span> 
							<span v-else>{{item.damage_name}}({{item.typ_name}})</span>
						</td>		
		...
		<div class="text-right" v-if="viewModel.viewType!='person'"> <small>※ 호우는 재난시작일이 검색조건입니다.</small></div>	
	</div>
		...
		<div class="section-title"><ul><li>{{viewModel.selectedDamageGungu.sigungu}}의 호우 피해현황</li></ul></div>
		<div class="table-basic">
			<table>
...
<script	src="${pageContext.request.contextPath}/contents/advis/js/model/vue/disasterDmeRain-controller.min.js?v=20190403"></script>
